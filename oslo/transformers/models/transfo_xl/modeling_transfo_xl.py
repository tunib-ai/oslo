...
try:
    ...
    from transformers.models.transfo_xl.modeling_transfo_xl_utilities import (
...
class PositionwiseFF(nn.Module):
    def __init__(
        ...
        self.CoreNet = nn.ModuleList(
            [
                onn.Linear(d_model, d_inner, skip_bias_add=True),
                onn.FusedBiasGeLU(),
                nn.Dropout(dropout),
                onn.Linear(d_inner, d_model, skip_bias_add=True),
                onn.FusedBiasDropout(dropout),
            ]
        )
    ...
    def forward(self, inp):
        if self.pre_lnorm:
            core_out, bias = self.CoreNet[0](self.layer_norm(inp))
            core_out = self.CoreNet[1](core_out, bias)
            core_out = self.CoreNet[2](core_out)
            core_out, bias = self.CoreNet[3](core_out)
            core_out = self.CoreNet[4](core_out, bias)

            # residual connection
            output = core_out + inp
        else:
            # positionwise feed-forward
            core_out, bias = self.CoreNet[0](inp)
            core_out = self.CoreNet[1](core_out, bias)
            core_out = self.CoreNet[2](core_out)
            core_out, bias = self.CoreNet[3](core_out)
            core_out = self.CoreNet[4](core_out, bias)
...
class RelPartialLearnableMultiHeadAttn(nn.Module):
    ...
    def forward(
        ...
        # Create attn_weigths and attention_mask to apply fused scale mask softmax
        # [bsz x n_head x qlen x klen]
        attn_weights = attn_score.permute(2, 3, 0, 1).contiguous()
        if attn_mask.dim() == 2:
            # [1 x 1 x qlen x klen]
            attention_mask = attn_mask[None, None, :, :]
        if attn_mask.dim() == 3:
            # [1 x 1 x qlen x klen]
            attention_mask = attn_mask[:, :, :, None].permute(2, 3, 0, 1).contiguous()
        ...
        if F._is_fused_scale_mask_softmax_available(
            ...
            # [qlen x klen x bsz x h_head]
            attn_prob = (
                F._fused_scale_mask_softmax_cuda(
                    input=attn_weights,
                    scale=1.0,
                    use_triang_mask=False,
            ...
            if attn_mask is not None and torch.sum(attn_mask).item():
                ...
                if attn_mask.dim() == 2:
                    if next(self.parameters()).dtype == torch.float16:
                        attn_score = (
                            attn_score.float()
                            .masked_fill(attn_mask[None, :, :, None], -65000)
                            .type_as(attn_score)
                        )
                    else:
                        attn_score = (
                    ...
                    if next(self.parameters()).dtype == torch.float16:
                        attn_score = (
                            attn_score.float()
                            .masked_fill(attn_mask[:, :, :, None], -65000)
                            .type_as(attn_score)
                        )
                    else:
                        attn_score = (
...
class TransfoXLPreTrainedModel(PreTrainedModel, OsloModel):
    """
