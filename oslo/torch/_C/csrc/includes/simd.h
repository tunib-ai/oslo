...
#define SIMD_LOAD2(x, h)                                                       \
  ((h) ? _mm512_cvtph_ps(_mm256_loadu_si256((const __m256i *)x))               \
       : _mm512_loadu_ps(x))
#define SIMD_STORE2(x, d, h)                                                   \
  ((h) ? _mm256_store_ps(x, _mm256_castsi256_ps(_mm512_cvtps_ph(               \
...
#define SIMD_WIDTH 8
#define SIMD_LOAD2(x, h)                                                       \
  ((h) ? _mm256_cvtph_ps(_mm_loadu_si128((const __m128i *)x))                  \
       : _mm256_loadu_ps(x))

#define SIMD_STORE2(x, d, h)                                                   \
  ((h) ? _mm_store_ps(x, _mm_castsi128_ps(                                     \
                             _mm256_cvtps_ph(d, _MM_FROUND_TO_NEAREST_INT)))   \
...
template <int span>
inline void simd_fma(AVX_Data *dst, AVX_Data *src_m_l, AVX_Data src_m_r,
                     AVX_Data *src_a) {
#pragma unroll
  for (size_t i = 0; i < span; ++i) {
...
template <int span> inline void simd_sqrt(AVX_Data *dst, AVX_Data *src) {
#pragma unroll
  for (size_t i = 0; i < span; ++i) {
