...
multi_tensor_l2norm_mp_cuda(int chunk_size, at::Tensor noop_flag,
                            std::vector<std::vector<at::Tensor>> tensor_lists,
                            at::optional<bool> per_tensor_python);

using MATH_T = float;

template <typename T, typename param_t> struct LAMBStage1Functor {
...
#pragma unroll
        for (int ii = 0; ii < ILP; ii++) {
          l_p[ii] = r_p[ii];
          // Difference from APEX's LAMB kernel. `g` and `p` can be different
          // dtypes.
          l_g[ii] = r_p[ii];
          l_m[ii] = r_m[ii];
          l_v[ii] = r_v[ii];
...
template <typename T, int N, typename param_t> struct LAMBStage2Functor {
  static_assert((N == 2 && std::is_same<T, param_t>::value) ||
                    (N == 3 && std::is_same<param_t, float>::value),
                "");
  __device__ __forceinline__ void
  operator()(int chunk_size, volatile int *noop_gmem, TensorListMetadata<N> &tl,
    ...
    // to make things simple, we put aligned case in a different code path
    bool can_use_aligned_path = n % ILP == 0 && chunk_size % ILP == 0 &&
                                is_aligned(p) && is_aligned(update);
    if (N == 3) {
...
void multi_tensor_lamb_mp_cuda(
  ...
  std::vector<std::vector<at::Tensor>> stage1_tensor_lists(
      tensor_lists.begin(), tensor_lists.begin() + 4);
  ...
  auto param_norm_tuple =
      multi_tensor_l2norm_mp_cuda(chunk_size, noop_flag, param_list, true);
  ...
  if (n_tensors == 4) {
    DISPATCH_FLOAT_AND_HALF(
        ...
        multi_tensor_apply<4>(
            BLOCK_SIZE, chunk_size, noop_flag, stage1_tensor_lists,
            LAMBStage1Functor<scalar_t_0, scalar_t_0>(), beta1, beta2,
            beta3, // 1-beta1 or 1 depends on averaging mode
            // bias_correction1,
            // bias_correction2,
            step.data_ptr<int>(), bias_correction, epsilon, (adamMode_t)mode,
            weight_decay, global_grad_norm.data_ptr<float>(),
            max_grad_norm.data_ptr<float>(), found_inf.data_ptr<float>(),
  ...
  auto update_norm_tuple =
      multi_tensor_l2norm_mp_cuda(chunk_size, noop_flag, grad_list, true);
  ...
  if (n_tensors == 4) {
    DISPATCH_FLOAT_AND_HALF(
        ...
        multi_tensor_apply<2>(BLOCK_SIZE, chunk_size, noop_flag,
                              grad_param_list,
    ...
    DISPATCH_FLOAT_AND_HALF(
        tensor_lists[0][0].scalar_type(), 0, "lamb_stage_2",
        multi_tensor_apply<3>(BLOCK_SIZE, chunk_size, noop_flag,
                              grad_param_list,
