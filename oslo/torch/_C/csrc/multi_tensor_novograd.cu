...
typedef enum {
  MOMENT_MODE_0 = 0, // Novograd paper mode, momentum calculation with denom
                     // then decay inside
...
template <typename T> struct NovoGradFunctor {
...
void multi_tensor_novograd_cuda(
    ...
    const int grad_averaging, const int moment_mode, const int norm_type) {
  using namespace at;
  ...
  // L-inf: gn = a * gn + b * n
  multi_tensor_norm_out_cuda(chunk_size, noop_flag, grad_list, grad_norms,
                             beta2, (1.0f - beta2), norm_type);

  // Assume single type across p,g,m1,m2 now
  DISPATCH_DOUBLE_FLOAT_AND_HALF(
      tensor_lists[0][0].scalar_type(), 0, "novograd",
      multi_tensor_apply<3>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                            NovoGradFunctor<scalar_t_0>(), beta1, beta2,
                            beta3, // 1-beta1 or 1 depends on averaging mode
                            bias_correction1, bias_correction2, epsilon, lr,
                            (momentMode_t)moment_mode, weight_decay,
