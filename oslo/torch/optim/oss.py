...
class OSS(Optimizer):
    ...
    def __init__(
        ...
        **default: Any,
        ...
        self.group = parallel_context.get_group(ParallelMode.DATA)
    ...
    def step(
        ...
        with profiler.record_function("fairscale::oss::refresh_trainable"):
            if (
                self._default_device.type
                != self.param_groups[0]["params"][0].device.type
            ):
                logging.info(
    ...
    def clip_grad_norm(
        ...
        with profiler.record_function("fairscale::oss::clip_grad_norm"):
            ...
            local_params = (
                filter_params_fn(self._local_params)
                if filter_params_fn is not None
                else self._local_params
            )

            local_norm = calc_grad_norm(local_params, norm_type).to(
            ...
            if clip_coef < 1:
                for device, device_params in self._per_device_params.items():
                    for p in filter(
                        lambda x: x.grad is not None, device_params[self.rank]
                    ):
            ...
            for param_group in self.param_groups:
                for param in param_group["params"]:
                    ...
                    if self.__per_device_params.get(device) is None:
                        self.__per_device_params[device] = [
                            [] for _ in range(self.world_size)
                        ]
                    self.__per_device_params[device][self._param_to_rank[param]] += [
                        param
                    ]
