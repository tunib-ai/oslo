...
from oslo.torch.distributed.nn.functional import all_gather, all_reduce, reduce_scatter, broadcast, reduce
    ...
    def backward(ctx: Any, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            weight_grad = torch.matmul(
                output_grad.reshape(-1, output_grad.shape[-1]).transpose(0, 1), inputs.reshape(-1, inputs.shape[-1]))
...
class _Classifier3D(torch.autograd.Function):
    ...
    def backward(ctx, output_grad: Tensor) -> Tuple[Tensor, ...]:
        ...
        with torch.no_grad():
            ...
            if ctx.parallel_context.get_local_rank(ctx.input_parallel_mode) == ctx.parallel_context.get_local_rank(ctx.output_parallel_mode):
                weight_grad, op = all_reduce(weight_grad, ctx.parallel_context, ctx.weight_parallel_mode, async_op=True)
                async_ops.append(op)
            else:
                weight_grad = None
...
def split_tensor_3d(
    ...
    if tensor.size(dim) <= 1:
        return tensor
    output = torch.chunk(tensor, parallel_context.get_world_size(parallel_mode),
                         dim=dim)[parallel_context.get_local_rank(parallel_mode)].contiguous()
    return output


def split_batch_3d(inputs: Tensor,
                   dim: int = 0,
                   parallel_context: Optional[ParallelContext] = None,
                   input_parallel_mode: ParallelMode = ParallelMode.TENSOR_3D_INPUT,
                   weight_parallel_mode: ParallelMode = ParallelMode.TENSOR_3D_WEIGHT) -> Tensor:
    ...
    if inputs.size(dim) <= 1:
        return inputs
    output = torch.chunk(inputs, weight_world_size,
                         dim=dim)[parallel_context.get_local_rank(weight_parallel_mode)].contiguous()
    output = torch.chunk(output, input_world_size,
                         dim=dim)[parallel_context.get_local_rank(input_parallel_mode)].contiguous()
    return output
...
class _ReduceTensor3D(torch.autograd.Function):
    ...
    def forward(ctx: Any, inputs: Tensor, parallel_context: ParallelContext, parallel_mode: ParallelMode):
        return all_reduce(inputs, parallel_context=parallel_context, parallel_mode=parallel_mode)
...
def reduce_tensor_3d(tensor: Tensor, parallel_context: ParallelContext, parallel_mode: ParallelMode) -> Tensor:
...
def all_gather_tensor_3d(tensor: Tensor, dim: int, parallel_context: ParallelContext, parallel_mode: ParallelMode) -> Tensor:
...
def reduce_scatter_tensor_3d(tensor: Tensor, dim: int, parallel_context: ParallelContext, parallel_mode: ParallelMode) -> Tensor:
...
class _ReduceByBatch3D(torch.autograd.Function):
    ...
    def forward(ctx: Any,
        ...
        if reduce_mean:
            reduce_size = parallel_context.get_world_size(input_parallel_mode) * parallel_context.get_world_size(weight_parallel_mode)
            output /= reduce_size
            if ctx:
...
def reduce_by_batch_3d(tensor: Tensor,
    ...
    Note:
        ...
        in `parallel_mode <https://github.com/hpcaitech/ColossalAI/blob/main/colossalai/context/parallel_mode.py>`_
    """
    return _ReduceByBatch3D.apply(tensor, reduce_mean, parallel_context, input_parallel_mode, weight_parallel_mode)
...
class _BroadcastWeight3D_FromDiagonal(torch.autograd.Function):
    ...
    def forward(
        ...
        ranks_in_group = parallel_context.get_ranks_in_group(input_parallel_mode)
        src_rank = ranks_in_group[parallel_context.get_local_rank(output_parallel_mode)]
        output = broadcast(inputs, src_rank, parallel_context=parallel_context, parallel_mode=input_parallel_mode)
        if ctx:
            ctx.src_rank = src_rank
            ctx.parallel_context = parallel_context
            ctx.input_parallel_mode = input_parallel_mode
