...
if TYPE_CHECKING:
    from oslo.torch.nn.parallel.data_parallel import FullyShardedDataParallel
...
def check_param_counts_before_sharding(
    full_optim_state_dict: Dict, n_instances: int
) -> None:
...
def _extract_non_tensor_state(
    combined_state: Dict[int, Dict[str, List]], param_id: int
) -> Dict:
    non_tensor_state = (
        {}
    )  # This state is like the `step` count in Adam, not a tensor so we dont unpad or cat it.
    for k, v in combined_state[param_id].items():
        if torch.is_tensor(v[0]):
            ...
            raise TypeError(
                f"Dont know how to consolidate optimizer param {k} with values {v}"
            )
...
def _unflatten_optim_state(
    ...
    non_tensor_state = {
        id: _extract_non_tensor_state(combined_state, id) for id in combined_state
    }
    ...
    for local_id, num_unflat in enumerate(num_global_params):
        for _ in range(num_unflat):
            ...
            global_to_local_id[next_global_id] = (
                local_id if local_id in local_ids else None
            )
            next_global_id += 1
    ...
    unflat_state = {
        i: copy.deepcopy(non_tensor_state[0]) for i in range(sum(num_global_params))
    }
    ...
    for local_id in local_ids:
        ...
        for k, v in combined_state[local_id].items():
            ...
            if instance_list[local_id].flatten_parameters:
                ...
                from oslo.torch.nn.parallel import FlattenParamsWrapper

                param_views: Iterator = cast(
                    FlattenParamsWrapper, instance_list[local_id]
                ).get_param_views([flat_buffer])
                for global_id, param_view in zip(
                    sorted(local_to_global[local_id]), param_views
                ):
                    assert (
                        k not in unflat_state[global_id]
                    ), f"already added {k} to {global_id} {local_id}"
                    unflat_state[global_id][k] = param_view
            else:
                # Copy non-flatten state directly.
                assert (
                    len(local_to_global[local_id]) == 1
                ), "Only support a single non-flatten parameter"
...
def build_unflat_state_dict(
    ...
    for local_id, v in uncollected_opt_state.items():
        ...
        state[local_id] = {
            buffer_name: [x]
            for buffer_name, x in v.items()
            if not is_singleton_tensor(x)
        }
        singleton_state[local_id] = {
            buffer_name: [x] for buffer_name, x in v.items() if is_singleton_tensor(x)
        }
    # local ids are in the current state, global_ids will be in returned state.
    unflat_state, global_to_local_id = _unflatten_optim_state(
        state, instance_list, world_pad_info, singleton_state
    )
