...
def get_process_group_cached(
    name: ProcessGroupName = ProcessGroupName.default,
    ...
    if not dist.is_initialized():
        ...
        if name == ProcessGroupName.reduce_scatter and "pytest" in sys.modules:
            ...
            raise RuntimeError(
                "torch.distributed is not yet initialized but process group is requested."
            )
    ...
    if not hasattr(get_process_group_cached, "_global_group_cache"):
        ...
        cache[
            (ProcessGroupName.default, frozenset(list(range(dist.get_world_size()))))
        ] = default_pg
