...
def unpack_kwargs(
    kwarg_keys: Tuple[str, ...], flat_args: Tuple[Any, ...]
...
def unpack_non_tensors(
    ...
    packed_non_tensors: Optional[Dict[str, List[Any]]],
) -> Tuple[Any, ...]:
    ...
    assert len(tensors) + len(objects) == len(is_tensor_list), (
        f"len(tensors) {len(tensors)} len(objects) {len(objects)} "
        f"len(is_tensor_list) {len(is_tensor_list)}"
    )
    obj_i = tnsr_i = 0
