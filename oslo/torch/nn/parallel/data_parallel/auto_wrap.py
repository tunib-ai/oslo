...
def default_auto_wrap_policy(
    ...
    if recurse:
        ...
        return (
            (module_is_root and skip_params_check_for_root) or is_large
        ) and not isinstance(module, tuple(exclude_wrap_modules))
...
def enable_wrap(
    auto_wrap_policy: Optional[Callable] = None, **wrapper_kwargs: Any
...
def auto_wrap(
    module: nn.Module, auto_wrap_policy: Optional[Callable] = None, **kwargs: Any
) -> nn.Module:
    """
...
class ConfigAutoWrap:
    ...
    def __init__(
        self, auto_wrap_policy: Optional[Callable] = None, **kwargs: Dict[str, Any]
    ):
        self.auto_wrap_policy = auto_wrap_policy
        self.kwargs = kwargs

    @staticmethod
    def enable_autowrap_context(
        auto_wrap_policy: Optional[Callable], kwargs: Any
    ) -> None:
        if ConfigAutoWrap.in_autowrap_context:
        ...
        assert "wrapper_cls" in kwargs.parallel_keys()
        ConfigAutoWrap.wrapper_cls = cast(Callable, kwargs["wrapper_cls"])
        del kwargs["wrapper_cls"]
        # Save the rest.
        ConfigAutoWrap.auto_wrap_policy = (
            default_auto_wrap_policy if auto_wrap_policy is None else auto_wrap_policy
        )
