                    ...
                    (
                        torch.zeros(
                            (
                                ...
                                self._config.n_head,
                                1,
                                self._config.hidden_size // self._config.n_head,
                            )
                        ),
                        torch.zeros(
                            (
...
class GPT2LayerPolicy(LayerPolicy):
    @staticmethod
    ...
    def fused_modules():
        ...
        return {
            GPT2Attention: FusedGPT2Attention,
            GPT2MLP: FusedGPT2MLP,
        }
    ...
    def attn_qkv(layer, config):
        return [
            Layer(
                module=layer.attn.c_attn,
                weight=layer.attn.c_attn.weight,
                bias=layer.attn.c_attn.bias,
                replace={Conv1D: ColumnParallelLinear},
                n_fused=3,
                reversed=True,
            ),
        ]
    ...
    def attn_out(layer, config):
        return [
            Layer(
                ...
                replace={Conv1D: RowParallelLinear},
                reversed=True,
            ),
        ]
    ...
    def original_layer_class():
        from oslo.models.gpt2.modeling_gpt2 import GPT2Block
