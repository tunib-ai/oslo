...
import oslo.torch.optim as optim
...
class TestSingleRank(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gpc = ParallelContext.from_torch(
...
def run_test_collect_shards(rank, world_size, reference_rank, tempfile_name):
    ...
    optimizer.load_state_dict(optimizer_state_dict)
    _ = optimizer.step(closure=closure)
    check_same_models_across_ranks(model, gpc.get_group(ParallelMode.DATA), params_should_be_equal=True, check_broadcast_buffers=False)
...
def run_test_multiple_groups(rank, world_size, tempfile_name):
    ...
    gpc.add_group(parallel_mode=ParallelMode.DATA, group=process_group)
