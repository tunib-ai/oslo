...
w = linear.weight.data.chunk(2, dim=1)[parallel_context.get_local_rank(ParallelMode.TENSOR_2D_ROW)]
w = w.chunk(2, dim=0)[parallel_context.get_local_rank(ParallelMode.TENSOR_2D_COL)]

# split bias into 0:[0], 1:[2], 2:[1], 3:[3]
b = linear.bias.data.chunk(2, dim=0)[parallel_context.get_local_rank(ParallelMode.TENSOR_2D_ROW)]
