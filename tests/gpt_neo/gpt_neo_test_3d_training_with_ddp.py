...
class Test3DTrainingWithDDP:
    ...
    def test_gpt_neo_lm_head_model(self):
        ...
        model_1d_ddp = DistributedDataParallel(
            model_1d,
            process_group=model_3d.mpu.get_data_parallel_group(),
            device_ids=[curr_device],
            output_device=curr_device,
        )

        model_3d_ddp = DistributedDataParallel(
            model_3d.gpu_modules(),
            process_group=model_3d.mpu.get_data_parallel_group(),
            device_ids=[curr_device],
            output_device=curr_device,
        )
        ...
        data_loader = DataLoader(
            ...
            sampler=DistributedSampler(
                datasets,
                num_replicas=model_3d.mpu.get_data_parallel_world_size(),
                rank=model_3d.mpu.get_data_parallel_rank(),
                shuffle=False,
            ),
        )
    ...
    def test_gpt_neo_for_sequence_classification(self):
        ...
        model_1d = GPTNeoForSequenceClassification.from_pretrained(
...
if __name__ == "__main__":
    test = Test3DTrainingWithDDP(num_gpus=8, batch_size=16)
