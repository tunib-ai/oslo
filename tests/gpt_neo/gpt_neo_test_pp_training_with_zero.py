...
from oslo.models.gpt_neo.modeling_gpt_neo import GPTNeoForCausalLM
...
class TestPPTrainingWithZeRO:
    ...
    def test_gpt_neo_lm_head_model(self):
        model_pp = GPTNeoForCausalLM.from_pretrained_with_parallel(
            "EleutherAI/gpt-neo-125M", pipeline_parallel_size=self.num_gpus // 2
        )
        ...
        for i, data in enumerate(data_loader):
            ...
            if dist.get_rank() == 0:
                print(
                    f"GPTNeoForCausalLM - "
                    f"step: {i}, loss_1d:{loss_1d}, loss_pp:{loss_pp}"
                )
