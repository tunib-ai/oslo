...
def test_bert_pretraining(model_name, max_length, dataset, mlm_probability=0.15):
    print(f"---- {model_name} test ----\n")
    processor = ProcessorForBertPretraining(model_name, max_length)
    ...
    data_collator = DataCollatorForBertPretraining(processor, mlm_probability=mlm_probability)
...
def test_sp_collator(
    ...
    sp_data_collator = DataCollatorForBertPretraining(processor, parallel_context=parallel_context)
...
if "__main__" == __name__:
    ...
    processor, processed_dataset = test_bert_pretraining("bert-base-cased", 512, dataset)
    processor, processed_dataset = test_bert_pretraining("bert-base-cased", 512, dataset, 0.3)
    # test_sp_collator(processor, processed_dataset)
    processor, processed_dataset = test_bert_pretraining("klue/bert-base", 512, dataset)
    # test_sp_collator(processor, processed_dataset)