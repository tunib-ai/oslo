...
class TestDataBertPretraining(TestDataBinarization):
    def __init__(
        ...
        self.data_collator = DataCollatorForBertPretraining(self.processor)
    ...
    def mask_ratio_check(self, dataloader):
        ...
        for batch in dataloader:
            ...
            assert(
                torch.isclose(mlm_probability, torch.tensor(self.data_collator.mlm_probability), atol=0.002)
            ...
            random_word_probability = torch.sum(masked_tokens != mask_token_id) / num_total
            assert(
...
if "__main__" == __name__:
    ...
    bert_test(512, dataset, mlm_probability=0.2)
    ...
    # bert_sp_test = TestDataBertPretraining("bert-base-cased", parallel_context)
