...
class TestTPTrainingWithFusion:
    ...
    def test_gptj_lm_head_model(self):
        ...
        model_tp = (
            GPTJForCausalLM.from_pretrained_with_parallel(
                "anton-l/gpt-j-tiny-random", tensor_parallel_size=self.num_gpus
            )
            .train()
            .fuse()
        )
        ...
        model_1d = (
            GPTJForCausalLM.from_pretrained("anton-l/gpt-j-tiny-random").cuda().train()
        )
...
if __name__ == "__main__":
    test = TestTPTrainingWithFusion(num_gpus=4, batch_size=4)
