...
def main():
    ...
    # reset_parapmeters need to first
    # model = model.half()
    ...
    if gpc.is_initialized(parallel_mode) and gpc.get_world_size(parallel_mode) > 1:
        for name, param in model.named_parameters():
            ...
            dist.broadcast(param, src=ranks[0],
    ...
    dist.barrier()
    print('scheduler', f'{rank}' * 30)

    # init
    engine, *dummy = initialize(model,
                                optimizer,
                                criterion,
                                )

    dist.barrier()
    print('engine', f'{rank}' * 30)
    ...
    for step in range(1, gpc.config.TRAIN_ITERS + 1):
        ...
        if step % gpc.config.EVAL_INTERVAL == 0:
            ...
            for j in range(gpc.config.EVAL_ITERS):
                with torch.no_grad():
                    if use_pipeline:
                        ...
                        lm_loss, sop_output = engine(tokens, padding_mask, types, lm_labels)
                        eval_loss = engine.criterion(lm_loss,
                                                     sop_output,
                                                     loss_mask,
                                                     sentence_order)
            ...
            # loss_scale = engine.optimizer.optim.get_loss_scale().item()   # does not work
            ...
            logger.info(
                ...
                # f'| Grad Norm: {grad_norm} | Skipped Iterations: {skip_iters} ' +
                # f'| Loss Scale: {loss_scale}' +
                # f"| Learning rate: {lr} | Num Zero in Grad: {num_zeros_in_grad} | " + timer_string,
                f'| Skipped Iterations: {skip_iters} ' +
