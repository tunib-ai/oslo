...
class BertForPretrain(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.seq_parallel_size = 1 if not gpc.is_initialized(ParallelMode.SEQUENCE) else gpc.get_world_size(ParallelMode.SEQUENCE)
    ...
    def forward(self, input_ids, attention_masks, tokentype_ids, lm_labels, ks_save_path=None):
        ...
        outputs.append(pos_ids.detach())
        outputs.append(attention_masks.detach())

        hidden_states = self.embedding(input_ids, pos_ids, tokentype_ids)

        outputs.append(hidden_states.detach())
        ...
        for idx, layer in enumerate(self.bert_layers):
            hidden_states, mid_outputs = layer(hidden_states, attention_masks, ks_save_path)
        ...
        return self.head(output, self.embedding.word_embedding_weight, lm_labels), outputs
