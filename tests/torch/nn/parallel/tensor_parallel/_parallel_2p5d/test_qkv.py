...
from oslo.torch.nn.parallel.tensor_parallel._parallel_2p5d._wrapper import _TensorParallel2p5D
from oslo.torch.nn import Linear2p5D
from oslo.torch.distributed import ParallelContext, ParallelMode
from copy import deepcopy
from _utils import gather_2d, gather_1d
...
# [t][f*t]
weight_list = _TensorParallel2p5D._deconstruct_combined_qkv(weight_list, tesseract_dim, fusion_degree, False)
bias_list = _TensorParallel2p5D._deconstruct_combined_qkv(bias_list, tesseract_dim, fusion_degree, True)
chunked_w = weight_list[row_rank][col_rank]
chunked_b = bias_list[row_rank]

linear_2p5d = Linear2p5D(4, fusion_degree*4, parallel_context=parallel_context, bias=True)
...
recon_chunked_w = gather_2d(parallel_context, linear_2p5d.weight.data, tesseract_dim, True)
...
recon_w = _TensorParallel2p5D._reconstruct_combined_qkv(recon_chunked_w, tesseract_dim, fusion_degree, False)
recon_b = _TensorParallel2p5D._reconstruct_combined_qkv(recon_chunked_b, tesseract_dim, fusion_degree, True)
