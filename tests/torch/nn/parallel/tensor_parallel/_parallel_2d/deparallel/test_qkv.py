...
weight_list = [
    weight.chunk(summa_dim * fusion_degree, dim=0) for weight in weight_list
]
...
linear_2d = Linear2p5D(4, fusion_degree*4, parallel_context=parallel_context, bias=True)
...
recon_w = _TensorParallel2p5D._reconstruct_combined_qkv(recon_chunked_w, summa_dim, fusion_degree, False)
recon_b = _TensorParallel2p5D._reconstruct_combined_qkv(recon_chunked_b, summa_dim, fusion_degree, True)
